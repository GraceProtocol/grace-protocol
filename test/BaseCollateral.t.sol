...
contract BaseCollateralTest is Test {
    ...
    function testDepositOnBehalf(uint amount) public {
        ...
        assertEq(baseCollateral.sharesSupply(), amount);
        assertEq(baseCollateral.sharesOf(address(1)), amount);
        assertEq(baseCollateral.getCollateralOf(address(1)), amount);
        assertEq(token.balanceOf(address(baseCollateral)), amount);
    }
    ...
    function testSecondDeposit(uint amount) public {
        ...
        assertEq(baseCollateral.sharesOf(address(this)), amount * 2);
        assertEq(baseCollateral.getCollateralOf(address(this)), amount * 2);
    ...
    function testDepositAfterPartialWithdrawal(uint amount) public {
        amount = bound(amount, 10, sqrtMaxUint / 2);
        baseCollateral.deposit(address(this), amount);
    ...
    function testWithdrawPartial(uint amount) public {
        ...
        assertEq(baseCollateral.sharesOf(address(this)), sqrtMaxUint - amount);
        assertEq(baseCollateral.getCollateralOf(address(this)), sqrtMaxUint - amount);
